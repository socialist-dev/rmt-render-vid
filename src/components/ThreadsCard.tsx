...
// SVG Icons Bar (#cccccc)
const HeartIcon = () => (
    <svg aria-label="Like" role="img" viewBox="-0.5 0 25 24" width="34px" height="34px" style={{ color: '#cccccc', display: 'block' }}>
...
const ReplyIcon = () => (
    <svg aria-label="Reply" role="img" viewBox="0 0 24 24" width="34px" height="34px" style={{ color: '#cccccc', display: 'block' }}>
...
const RepostIcon = () => (
    <svg aria-label="Repost" role="img" viewBox="0 0 24 24" width="34px" height="34px" style={{ color: '#cccccc', display: 'block' }}>
...
const ShareIcon = () => (
    <svg aria-label="Share" role="img" viewBox="0 0 24 24" width="34px" height="34px" style={{ color: '#cccccc', display: 'block' }}>
...
// Thanh Action Bar dùng chung
const ActionBar = ({ likes = '0', replies = '0', reposts = '0' }: any) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '36px', color: '#cccccc', paddingTop: '6px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <HeartIcon />
            {likes !== '0' && <span style={{ fontSize: '26px', fontWeight: '600', color: '#cccccc', lineHeight: 1 }}>{likes}</span>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <ReplyIcon />
            {replies !== '0' && <span style={{ fontSize: '26px', fontWeight: '600', color: '#cccccc', lineHeight: 1 }}>{replies}</span>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <RepostIcon />
            {reposts !== '0' && <span style={{ fontSize: '26px', fontWeight: '600', color: '#cccccc', lineHeight: 1 }}>{reposts}</span>}
        </div>
...
export const ThreadsCard: React.FC<any> = ({
    type = 'post', // Nhận diện rõ ràng: "post" (bài chính) hoặc "reply" (bài phản hồi)
    handle = 'username',
    avatar,
    text = '',
    mediaUrl,
    ...
    // 1. GIAO DIỆN BÀI CHÍNH (POST) - GIỮ NGUYÊN 100% LAYOUT GỐC BAN ĐẦU
    if (!isReply) {
        return (
            <div style={{
                background: '#101010',
                ...
                {/* Header: Avatar, Username, Follow, Logo */}
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '20px' }}>
                    {avatar ? (
                        <Img src={avatar} style={{ width: '68px', height: '68px', borderRadius: '50%', marginRight: '16px', objectFit: 'cover', display: 'block' }} />
                    ) : (
                        <div style={{ width: '68px', height: '68px', borderRadius: '50%', marginRight: '16px', backgroundColor: '#333' }} />
                    )}
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontWeight: '700', fontSize: '28px', color: '#f3f5f7' }}>{handle}</span>
                        <span style={{ color: '#777777', fontSize: '26px' }}>·</span>
                        <span style={{ color: '#777777', fontSize: '26px', fontWeight: '600', letterSpacing: '-0.2px' }}>Follow</span>
                    </div>
                    <ThreadsLogo />
                </div>

                {/* Content Text full bề ngang */}
                <div style={{ fontSize: '34px', lineHeight: 1.42, color: '#f3f5f7', marginBottom: mediaUrl ? '22px' : '26px', textAlign: 'left', fontWeight: '400', wordBreak: 'break-word', whiteSpace: 'pre-wrap' }}>
                    {text}
                </div>

                {/* Media Image full bề ngang */}
                {mediaUrl && (
                    <div style={{ width: '100%', borderRadius: '18px', overflow: 'hidden', border: '1px solid #282828', marginBottom: '26px', boxSizing: 'border-box' }}>
                        <Img src={mediaUrl} style={{ width: '100%', height: 'auto', maxHeight: '650px', objectFit: 'cover', display: 'block' }} />
                    </div>
                )}

                {/* Action Bar */}
                <ActionBar likes={likes} replies={replies} reposts={reposts} />
            </div>
        );
    }

    // 2. GIAO DIỆN BÀI PHẢN HỒI (REPLY) - BỐ CỤC 2 CỘT CÓ THANH NỐI DỌC #333638
    return (
        <div style={{
            position: 'relative',
            background: '#101010',
            ...
            {/* Cột trái: Avatar + Thanh nối dọc (#333638) */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginRight: '20px', flexShrink: 0 }}>
                {avatar ? (
                    <Img src={avatar} style={{ width: '68px', height: '68px', borderRadius: '50%', objectFit: 'cover', display: 'block' }} />
                ) : (
                    <div style={{ width: '68px', height: '68px', borderRadius: '50%', backgroundColor: '#333' }} />
                )}
                <div style={{ width: '3px', flex: 1, minHeight: '60px', backgroundColor: '#333638', marginTop: '14px', marginBottom: '6px', borderRadius: '2px' }} />
            </div>
